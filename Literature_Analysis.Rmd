...
dummy <- dummyVars(" ~ .", data = df_features_factor %>% dplyr::select(-c("Problem")))
newdata <- data.frame(predict(dummy, newdata = df_features_factor %>% dplyr::select(-c("Problem"))),
                      df_features_factor["Problem"])
```
...
```{r}
distance_matrix <- matrix(nrow = nrow(newdata), ncol = nrow(newdata))
for(i in 1:nrow(newdata)) {
  for(j in 1:nrow(newdata)) {
    distance_matrix[i, j] <- jaccard_distance(newdata[i, 1:(ncol(newdata) - 1)],
                                              newdata[j, 1:(ncol(newdata) - 1)])
  }
}
```
